  ...
  try {
    ...
    if (!g.ok) {
      const status = g.reason === "no_credits" ? 402
        ...
        : 403;
      if (g.reason === "no_credits") logAiCallBg({ fn: "refine-site", userId: user.id, siteId, model: MODEL, durationMs: Date.now() - startedAt, success: false, errorMessage: "no_credits", tokensIn: 0, tokensOut: 0, meta: { http_status: status, limit_hit_reason: "no_credits" } });
    ...
    logAiCallBg({ fn: "refine-site", userId: user.id, siteId, model: MODEL, tokensIn: data?.usage?.input_tokens ?? null, tokensOut: data?.usage?.output_tokens ?? null, durationMs: Date.now() - startedAt, success: true });
