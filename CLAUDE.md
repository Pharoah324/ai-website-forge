...
1. **✅ FIXED (test-verified June 18, 2026) — Plan change created duplicate/orphaned subscriptions → double-billing.** Old behavior: changing plans went through `create-checkout` → a NEW Checkout → a NEW subscription; the old sub was never canceled and `checkout.session.completed` overwrote `profiles.stripe_subscription_id`. **Fix shipped (single-subscription-per-customer model):**
   ...
   - **STILL TODO:** browser smoke test of the `change-subscription` invocation + the confirmation dialog (replicated via CLI in the sandbox, not yet clicked through the real UI) — do post-push.
...
3. **[Bug — decided] Cancel/downgrade does NOT cap credit balances to free.** The `customer.subscription.deleted` handler sets `plan='free'` + resets `monthly_*_limit` to 20/300, but leaves the current `build_credits`/`runtime_credits` at the paid balance (verified: canceled user kept 100/2500). **DECISION: cap credits to free limits (20/300) on cancel** — so this is a bug to fix alongside the cancel path (not intended generosity). Document only; not fixed yet.
